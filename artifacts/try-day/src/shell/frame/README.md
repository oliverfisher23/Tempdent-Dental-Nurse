...
This directory is the shell's frame around every task: the layout, HUD, map,
step guide, notepad and dialogue that any client's day runs inside. It reads
the client (places, routes, people, copy) through `useClient()`; nothing here
imports from `src/client/`, and `tests/boundary.test.ts` keeps it that way.
The `Kitchen*` names are legacy from the first client and will be renamed in a
later template pass.
...
- `scenes`: A map of PlaceId to ReactNode (the scene for that place). The frame manages crossfading between them. Every place in the task's route (`workplace.taskRoutes[id]`, including `start`) needs a scene, or the stage is black there.
...
## State & Rules
- **DO NOT put simulation rules in UI.** All `evaluate*` logic stays in the client's progress model (`src/client/lib/simulation.ts`).
- Update task state via `useProgress().updateTask(id, prev => next)` (client code imports the narrowed hook from `@client/lib/progress`).
