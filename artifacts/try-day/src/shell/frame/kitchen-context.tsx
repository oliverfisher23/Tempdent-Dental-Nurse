...
import type { PlaceId, DayLight, Person } from '@shell/lib/client';
...
export function KitchenProvider({ taskId, frozen = false, children }: { taskId: TaskId; frozen?: boolean; children: ReactNode }) {
  ...
  const { places: PLACES } = workplace;
  const routeConfig = workplace.taskRoutes[taskId];
  // A reload puts the learner back in the room they were in, with the same workspace open,
  // unless the task's route opts out (a round that resumes from its saved rows by itself,
  // where re-sending the action would open something the learner has not opened).
  const [saved] = useState(() => {
    const position = progress.positions[taskId];
    const place = position && routeConfig.places.includes(position.place) ? position.place : routeConfig.start;
    const restoresWorkspace = routeConfig.restoresWorkspace !== false;
  ...
  const goTo = useCallback((to: PlaceId) => {
    ...
    if (reduceMotion) {
      advanceClock(1);
      setPlace(to);
      kitchenAudio.setAmbience(PLACES[to].ambience);
      setMapPhase('closed');
      setArrivedAt(Date.now());
      return;
    }
  ...
  }, [frozen, place, mapPhase, routeConfig.places, PLACES, advanceClock, reduceMotion, later, enterRoom]);
