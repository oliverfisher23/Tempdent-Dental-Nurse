...
import { KitchenModal } from '@shell/frame/kitchen-modal';
import type { BriefingVideoId } from '@shell/lib/client';
...
/**
 * The mentor's filmed briefing for a task. While the film is still to come the
 * player is a placeholder frame and the words are written out underneath, so
 * nobody needs sound or video to carry on.
 */
export function BriefingVideoModal({ videoId, isOpen, onClose }: BriefingVideoModalProps) {
  const { briefingVideos, copy: { media: COPY } } = useClient();
  const video = briefingVideos[videoId];
...
export function BriefingVideoButton({ videoId, tone = 'light', label, size = 'sm', variant = 'outline', className }: BriefingVideoButtonProps) {
  const [open, setOpen] = useState(false);
  const { copy: { media: COPY } } = useClient();
  const dark = tone === 'dark';
  const buttonLabel = label ?? COPY.open;
  return (
    <>
      <Button
        type="button"
