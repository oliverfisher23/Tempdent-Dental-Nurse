...
import { briefingPatterns } from "@shell/lib/client";
...
import { PATTERN_ICONS } from "@shell/frame/how-to-card";
...
export function BriefingView({ name, setName, confirmReset, setConfirmReset }: BriefingViewProps) {
  ...
  const { brand, mentor, welcome: { copy: WELCOME_COPY }, copy: { patterns: PATTERN_COPY, media: MEDIA_ACCESSIBILITY_COPY }, mainBriefingVideo } = client;
  const { FRAME, TASK_ORDER, getTask } = client.day.spec;
  const BRIEFING_PATTERNS = briefingPatterns(client);
    ...
    <div data-testid="welcome-briefing" className="min-h-[100dvh] bg-background flex flex-col">
      <header className="px-6 md:px-10 py-5 flex items-center justify-between border-b border-border bg-foreground text-primary-foreground z-10 sticky top-0">
        <div className="flex items-center gap-4">
          <img src={brand.logo} alt={brand.logoAlt} className="h-6 md:h-8 object-contain" />
      ...
      <main id="main-activity" tabIndex={-1} className="flex-1 w-full max-w-6xl mx-auto px-4 sm:px-6 md:px-10 py-6 md:py-9 grid grid-cols-1 md:grid-cols-12 gap-7 md:gap-10 items-center">
        <div className="min-w-0 md:col-span-7 flex flex-col justify-center">
          <motion.div {...fade(0)} className="mb-5 flex items-center gap-4">
            {/* Two sizes, when the client supplies them, so the circle is shown close to 1:1. */}
            <img
              src={mentor.photo}
              srcSet={mentor.photo2x ? `${mentor.photo} 256w, ${mentor.photo2x} 512w` : undefined}
            ...
            />
            <div>
              <p className="font-bold text-xl text-foreground">{mentor.name}</p>
          ...
          <motion.div {...fade(0.15)} className="mt-5 space-y-3 border-t border-border pt-4">
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
              <BriefingVideoButton videoId={mainBriefingVideo} label={MEDIA_ACCESSIBILITY_COPY.openMain} />
