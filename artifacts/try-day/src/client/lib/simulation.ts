...
import {
  ...
  type DayRuntime,
  type Evaluation,
  type NotepadEntry as ShellNotepadEntry,
  type Progress as ShellProgress,
  type ProgressModel,
  type TaskPosition as ShellTaskPosition,
} from '@shell/lib/day';
import { deliveryReviewIssues, reconcileDeliveryUpdate } from './delivery-workflow';
...
// ---------------------------------------------------------------------------
// The spec: the kitchen's day document, run through the shell's generic day model
// ---------------------------------------------------------------------------
...
export type NotepadEntry = Omit<ShellNotepadEntry, 'taskId'> & { taskId: TaskId };
export type TaskPosition = ShellTaskPosition;
export type Progress = ShellProgress<TaskStates>;
...
/** Backed by the shell runtime below; kept as a named export for tests and fixtures. */
export function initialProgress(): Progress {
  return day.initialProgress();
}

/** Session-only, evaluator-valid fixtures used by the opt-in designer panel. */
export function testProgress(target: TaskId | null | undefined, initial: () => Progress = initialProgress): Progress {
  const p = initial();
...
export function isDayComplete(p: Progress): boolean {
  return day.isDayComplete(p);
}

// ---------------------------------------------------------------------------
// Saved progress repairs (records written by earlier versions of this kitchen)
// ---------------------------------------------------------------------------

/**
 * Runs after a saved record has been merged over the initial shape. Order matters:
 * legacy redesign deletion first, then the delivery signature, then Terence's
 * pre-filled chart rows.
 */
export function reviveSaved(tasks: TaskStates, saved: { stored: Record<string, unknown>; completed: string[] }): TaskStates {
  for (const id of TASK_ORDER) {
    const stored = saved.stored[id];
    ...
    const completedLegacy = saved.completed.includes(id);
    ...
    if (isRecord(stored) && !stored.redesign && (completedLegacy || runningLegacyChill)) {
      delete (tasks[id] as { redesign?: unknown }).redesign;
    }
  }
  ...
  if (!saved.completed.includes('check-the-delivery-in') && delivery.signed && deliveryReviewIssues(delivery, false).length > 0) {
  ...
  if (!saved.completed.includes('check-the-dietary-list')) {
...
// ---------------------------------------------------------------------------
// The runtime: persistence and host messaging come from the shell
// ---------------------------------------------------------------------------
...
export const model: ProgressModel<TaskStates> = {
  ...
  // The three states mean different fixtures: undefined is a blank day at the
  // briefing, null is every task complete, a task id is that task in progress.
  // An unknown id opens a blank day rather than a completed one.
  testProgress: (target, initial) =>
    testProgress(
      target === null ? null : target !== undefined && TASK_ORDER.includes(target as TaskId) ? (target as TaskId) : undefined,
      initial as () => Progress,
    ),
  reviveSaved,
  reconcileUpdate: (id, prev, next) =>
    id === 'check-the-delivery-in'
      ? (reconcileDeliveryUpdate(prev as DeliveryState, next as DeliveryState) as typeof next)
      : next,
};
...
export const TEST_STORAGE_KEY = day.spec.TEST_STORAGE_KEY;

export function loadProgress(testMode?: boolean): Progress {
  return day.loadProgress(testMode);
}
...
export const notifyHost = day.notifyHost;
