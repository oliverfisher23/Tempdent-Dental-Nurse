            ...
            <div className="relative w-full max-w-xl border border-border border-t-4 border-t-primary bg-background p-6 shadow-2xl sm:p-8">
              ...
              <div className="mt-6 flex flex-col gap-6">
                <div className="space-y-2 border p-4 rounded-md bg-card">
                  <span className="font-semibold block">Aftercare Advice</span>
                  <p className="text-sm text-muted-foreground mb-2">What must Karim know about Amira's numb mouth?</p>
                  <div className="flex flex-col gap-2">
                    <Button variant={state.aftercare === 'numbness' ? 'default' : 'outline'} disabled={frozen} className="justify-start h-auto text-left py-2" onClick={() => updateTask('reset', p => ({ ...p, aftercare: 'numbness'}))}>
                      "Her lip will be numb for a few hours. Be careful she doesn't bite it."
                    </Button>
                    <Button variant={state.aftercare === 'brushing' ? 'default' : 'outline'} disabled={frozen} className="justify-start h-auto text-left py-2" onClick={() => updateTask('reset', p => ({ ...p, aftercare: 'brushing'}))}>
                ...
                <div className="space-y-2 border p-4 rounded-md bg-card">
                  <span className="font-semibold block">Surgery Decontamination Wipe-down</span>
                  <p className="text-sm text-muted-foreground mb-2">Which sequence is correct for wiping down the work surfaces?</p>
                  <div className="flex flex-col gap-2">
                    <Button variant={state.wipeDown === 'complete' ? 'default' : 'outline'} disabled={frozen} className="justify-start h-auto text-left py-2" onClick={() => updateTask('reset', p => ({ ...p, wipeDown: 'complete'}))}>
                      Wipe from clean areas to dirty areas, then dispose of wipes in clinical waste.
                    </Button>
                    <Button variant={state.wipeDown === 'wrong_order' ? 'default' : 'outline'} disabled={frozen} className="justify-start h-auto text-left py-2" onClick={() => updateTask('reset', p => ({ ...p, wipeDown: 'wrong_order'}))}>
