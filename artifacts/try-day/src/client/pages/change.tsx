            ...
            <div className="relative w-full max-w-xl border border-border border-t-4 border-t-primary bg-background p-6 shadow-2xl sm:p-8">
              ...
              <div className="mt-6 flex flex-col gap-6">
                <div className="space-y-2 border p-4 rounded-md bg-card">
                  <span className="font-semibold block">What is your most urgent priority?</span>
                  <div className="flex flex-col gap-2">
                    <Button variant={state.firstPriority === 'stock' ? 'default' : 'outline'} disabled={frozen} className="justify-start h-auto text-left py-2" onClick={() => updateTask('change', p => ({ ...p, firstPriority: 'stock'}))}>
                      Move the delivery boxes into the stock room so they are out of the way.
                    </Button>
                    <Button variant={state.firstPriority === 'emergency' ? 'default' : 'outline'} disabled={frozen} className="justify-start h-auto text-left py-2" onClick={() => updateTask('change', p => ({ ...p, firstPriority: 'emergency'}))}>
                      Ensure Surgery 2 is set up with an emergency tray for Mr Nowak.
                    </Button>
                    <Button variant={state.firstPriority === 'decon' ? 'default' : 'outline'} disabled={frozen} className="justify-start h-auto text-left py-2" onClick={() => updateTask('change', p => ({ ...p, firstPriority: 'decon'}))}>
