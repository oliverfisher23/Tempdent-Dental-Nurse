            ...
            <div className="relative w-full max-w-xl border border-border border-t-4 border-t-primary bg-background p-6 shadow-2xl sm:p-8">
              ...
              <div className="mt-6 flex flex-col gap-6">
                <div className="space-y-2 border p-4 rounded-md bg-card">
                  <span className="font-semibold block">Water Line Flushing (Start of Day)</span>
                  <div className="flex gap-2">
                    {[20, 60, 120].map(sec => (
                      <Button
                        key={sec}
                        variant={state.waterFlushMinutes === (sec === 120 ? 2 : sec) ? "default" : "outline"}
                        disabled={frozen}
                ...
                <div className="space-y-2 border p-4 rounded-md bg-card">
                  <span className="font-semibold block">Emergency Kit Verification</span>
                  <div className="flex flex-col gap-2">
                    <Button
                      variant={state.emergencyKit === 'ignored' ? "default" : "outline"}
                      disabled={frozen}
                    ...
                    <Button
                      variant={state.emergencyKit === 'checked' ? "default" : "outline"}
                      disabled={frozen}
