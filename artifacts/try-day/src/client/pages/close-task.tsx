          ...
          <section className="absolute inset-0 flex items-center justify-center overflow-hidden bg-zinc-900 px-6 py-12 text-foreground">
            ...
            <div className="relative w-full max-w-xl border border-border border-t-4 border-t-primary bg-background p-6 shadow-2xl sm:p-8">
              ...
              <div className="mt-6 flex flex-col gap-6">
                <div role="group" aria-labelledby="close-task-q1" className="space-y-2 border p-4 rounded-md bg-card">
                  <span id="close-task-q1" className="font-semibold block">Greet Graham</span>
                  <div className="flex flex-col gap-2">
                    <Button variant={state.greeting === 'judgmental' ? 'default' : 'outline'} aria-pressed={state.greeting === 'judgmental'} disabled={frozen} className="justify-start h-auto text-left py-2" onClick={() => updateTask('close', p => ({ ...p, greeting: 'judgmental'}))}>
                      "Graham? You haven't been here in ten years. Come on through."
                    </Button>
                    <Button variant={state.greeting === 'calm_no_judgment' ? 'default' : 'outline'} aria-pressed={state.greeting === 'calm_no_judgment'} disabled={frozen} className="justify-start h-auto text-left py-2" onClick={() => updateTask('close', p => ({ ...p, greeting: 'calm_no_judgment'}))}>
        ...
        surgery2: (
          <section className="absolute inset-0 flex items-center justify-center overflow-hidden bg-zinc-900 px-6 py-12 text-foreground">
            ...
            <div className="relative w-full max-w-xl border border-border border-t-4 border-t-primary bg-background p-6 shadow-2xl sm:p-8">
              ...
              <div className="mt-6 flex flex-col gap-6">
                <div role="group" aria-labelledby="close-task-q2" className="space-y-2 border p-4 rounded-md bg-card">
                  <span id="close-task-q2" className="font-semibold block">Log the charting</span>
                  <div className="flex flex-col gap-2">
                    <Button variant={state.charting === 'logged' ? 'default' : 'outline'} aria-pressed={state.charting === 'logged'} disabled={frozen} className="justify-start h-auto text-left py-2" onClick={() => updateTask('close', p => ({ ...p, charting: 'logged'}))}>
                      Select UR6 and mark Occlusal Composite in the digital record.
                    </Button>
                    <Button variant={state.charting === 'wrong' ? 'default' : 'outline'} aria-pressed={state.charting === 'wrong'} disabled={frozen} className="justify-start h-auto text-left py-2" onClick={() => updateTask('close', p => ({ ...p, charting: 'wrong'}))}>
