          ...
          <section className="absolute inset-0 flex items-center justify-center overflow-hidden bg-zinc-900 px-6 py-12 text-foreground">
            ...
            <div className="relative w-full max-w-xl border border-border border-t-4 border-t-primary bg-background p-6 shadow-2xl sm:p-8">
              ...
              <div className="mt-6 flex flex-col gap-6">
                <div role="group" aria-labelledby="welcome-q1" className="space-y-2 border p-4 rounded-md bg-card">
                  <span id="welcome-q1" className="font-semibold block">How do you greet Amira?</span>
                  <div className="flex flex-col gap-2">
                    <Button variant={state.greeting === 'child_focused' ? 'default' : 'outline'} aria-pressed={state.greeting === 'child_focused'} disabled={frozen} className="justify-start h-auto text-left py-2" onClick={() => updateTask('welcome', p => ({ ...p, greeting: 'child_focused'}))}>
                      "Hi Amira, I'm {progress.studentName || 'your nurse'}. We're going to look after you today. Come on through."
                    </Button>
                    <Button variant={state.greeting === 'parent_focused' ? 'default' : 'outline'} aria-pressed={state.greeting === 'parent_focused'} disabled={frozen} className="justify-start h-auto text-left py-2" onClick={() => updateTask('welcome', p => ({ ...p, greeting: 'parent_focused'}))}>
                      "Mr Hassan? Dr Reid is ready for Amira now."
                    </Button>
                  </div>
                </div>

                <div role="group" aria-labelledby="welcome-q2" className="space-y-2 border p-4 rounded-md bg-card">
                  <span id="welcome-q2" className="font-semibold block">Reply to Karim's question</span>
                  <div className="flex flex-col gap-2">
                    <Button variant={state.dadQuestion === 'wait' ? 'default' : 'outline'} aria-pressed={state.dadQuestion === 'wait'} disabled={frozen} className="justify-start h-auto text-left py-2" onClick={() => updateTask('welcome', p => ({ ...p, dadQuestion: 'wait'}))}>
                      "It's better if parents wait outside so we have space."
                    </Button>
                    <Button variant={state.dadQuestion === 'allow_in' ? 'default' : 'outline'} aria-pressed={state.dadQuestion === 'allow_in'} disabled={frozen} className="justify-start h-auto text-left py-2" onClick={() => updateTask('welcome', p => ({ ...p, dadQuestion: 'allow_in'}))}>
