import { ADDED_GUESTS, ELENA_QUESTION, WASTE_BINS } from '@client/content/activities';
import type { StepGuide } from '@shell/copy/step-guide';
import { GUIDE_COPY } from '@shell/copy/step-guide';
import { COURSES, courseChecked, getDietaryRedesignStage } from '@client/lib/redesign-dietary';
import {
  evaluateClose,
  evaluateDietary,
  weightIsRight,
...
export function getDietaryGuide(state: DietaryState): StepGuide {
  ...
  if (stage === 'sheet') {
    return {
      ...
      actionLabel: 'Read the function sheet',
      place: 'pass',
  ...
  if (stage === 'chart') {
    ...
    return {
      ...
      actionLabel: 'Mark the allergen chart',
      place: 'events',
  ...
  if (stage === 'guests') {
    ...
    return {
      ...
      actionLabel: 'Decide the guests’ dishes',
      place: 'events',
  ...
  if (stage === 'board' || !evaluation.done) {
    return {
      ...
      actionLabel: 'Write the evening board',
      place: 'events',
  ...
  return {
    ...
    actionLabel: 'Look at the board again',
    place: 'events',
    action: 'dietary.open-board',
    pattern: 'tap',
  };
...
export function getCloseGuide(state: CloseState, chill: ChillState): StepGuide {
  ...
  for (let index = 0; index < WASTE_BINS.length; index += 1) {
    ...
    if (!state.weighed[bin.id]) {
      return {
        ...
        total: CLOSE_TOTAL,
        title: `Weigh ${binName}`,
        instruction: 'Choose a tub, press ‘Put it on the scales’, read the display, then type the weight on its row of the waste sheet.',
        actionLabel: 'Weigh the tub',
        place: 'pass',
  ...
  if (!handoverDone) {
    return {
      ...
      actionLabel: 'Write the handover',
      place: 'pass',
  ...
  if (!signaturesDone) {
    ...
    return {
      ...
      actionLabel: 'Sign the chill record with Terence',
      place: 'pass',
  ...
  return {
    ...
    title: GUIDE_COPY.complete,
    instruction: GUIDE_COPY.completeHint,
    actionLabel: 'Look at the chill record again',
    place: 'pass',
    action: 'close.open-elena',
    pattern: 'tap',
  };
