/**
 * Workplace sound: one AudioContext, three buses (ambient / ui / voice), a looped
 ...
 *
 * Usage from the UI:
 *   kitchenAudio.configure({ muteKey, ambience, ambienceLoop }); // once, before the first render
 *   kitchenAudio.unlock();                    // inside a click/tap handler (the "clock on" button)
 *   kitchenAudio.setAmbience(place.ambience); // when the student arrives somewhere
...
export type SoundName =
  ...
  | 'radio' // radio squelch before someone answers a call
  | 'scale'; // scales settling

/** How the ambience bed sounds in one place: lowpass cutoff (Hz) and gain. */
export interface ToneSpec {
  cutoff: number;
  gain: number;
}

/** The bed when a place has no ambience of its own. */
export const SILENT_TONE: ToneSpec = { cutoff: 1000, gain: 0 };

const DEFAULT_MUTE_KEY = 'springpod:try-day:sound-muted';
...
class KitchenAudio {
  ...
  private muteKey = DEFAULT_MUTE_KEY;
  private tone: ToneSpec = SILENT_TONE;
  /** A recorded room loop layered under the generated hum; none means the hum carries the room alone. */
  private ambienceLoop: string | null = null;
  private listeners = new Set<(muted: boolean) => void>();
  private eventTimer: ReturnType<typeof setTimeout> | null = null;

  constructor() {
    this.muted = readMutePreference(this.muteKey);
  ...
  /**
   * Give the saved mute choice a key of the client's own, so two try days on one
   * device do not share it, optionally the ambience the bed opens with, and the
   * URL of a recorded room loop to layer under the generated hum. Call before
   * the first render; the saved choice is re-read.
   */
  configure({ muteKey, ambience, ambienceLoop }: { muteKey: string; ambience?: ToneSpec; ambienceLoop?: string }) {
    if (ambience) this.setAmbience(ambience);
    this.ambienceLoop = ambienceLoop ?? null;
    if (muteKey === this.muteKey) return;
    this.muteKey = muteKey;
    this.muted = readMutePreference(muteKey);
  }
  ...
  setMuted(muted: boolean) {
    this.muted = muted;
    try {
      window.localStorage.setItem(this.muteKey, muted ? '1' : '0');
    } catch {
  ...
  setAmbience(tone: ToneSpec = SILENT_TONE) {
    this.tone = tone;
  ...
  private async startBed() {
    ...
    try {
      const res = this.ambienceLoop ? await fetch(this.ambienceLoop) : null;
      if (res?.ok) {
