...
export function getChillGuide(state: ChillState, started: boolean, waiting: boolean): StepGuide {
  ...
  if (state.trays.reduce((a, b) => a + b, 0) < PREP_SHEET.yourShareKg - 0.01) {
    return guide('chill-portion', 1, 'Portion the beef', 'Choose a tray, then hold Add beef to pour. Use the 0.5 kg buttons if you prefer. Empty your half of the pan into the trays.', 'chill:portion', 'Open the prep bench', 'tap');
  }
  if (!traysHaveSpace(state.shelfByTray)) {
    return guide('chill-load', 2, 'Load the chiller', 'Drag each tray from the trolley onto a shelf, or tap a tray and then tap a shelf. Leave an empty shelf between trays.', 'chill:chiller', 'Open the chiller', 'drag');
  }
  ...
  if (!started) {
    return guide('chill-start', 3, 'Start the chiller', 'The trays and probe are ready. Start the cycle at the control panel.', 'chill:chiller', 'Open the controls', 'tap');
  }
  ...
  if (previousWrong !== undefined) {
    return guide(`chill-correct-${previousWrong}`, 4, `Check the ${previousWrong}-minute reading`, 'Compare the chill record with your notebook and correct the reading before carrying on.', 'chill:chiller', 'Open the chiller', 'tap');
  }
  if (waiting) {
