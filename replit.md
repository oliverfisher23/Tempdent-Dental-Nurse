...
- `artifacts/try-day/src/` follows one dependency direction: `client -> shell -> kit`.
  ...
  - `src/client/scenes/README.md` — the stage engine contract for room scenes, speech, hotspots, close-up interactions, V2 stage layers (`find`, `path`, `controls`, `paced`), the panel slot, sound captions, answer handling and stable test ids.
  - `docs/Tempdent_TRY-Day_Storyboard_Dental-Nurse_V2_interaction-redesign.md` is the V2 interaction source of truth and `docs/V2-BUILD.md` the build brief and status; V1 answer keys stand where the storyboard keeps them and copy awaiting the SME is flagged `// TBC SME` in content. World sounds (`src/client/lib/sounds.ts`, `assets/sounds/`) are generated placeholders, each with a caption shown on the stage.
...
- `node tests/browser/run.mjs [names]` — one Playwright spec per task (`tests/browser/<task>.spec.mjs`) playing the real app end to end at 1280x720 and 390x844 through the designer test panel, plus `frame.spec.mjs` for the 390x480 and 800x480 embedded frame; needs the `artifacts/try-day: web` workflow running (the `browser-specs` check). Helpers live in `tests/browser/harness.mjs`.
