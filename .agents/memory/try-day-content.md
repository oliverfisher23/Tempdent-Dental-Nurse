...
- Instruments are read by the learner, never transcribed: the fridge round's dial thermometer and the delivery bench scales (kg lines only; counted units keep a plain text result) show the reading on the instrument and the learner types it. The scales' LCD reads two decimals ("8.00 kg"), so browser checks expect that form and confirm the number is not printed anywhere visible (screen-reader-only sentence in the status line is the one exception).
  **Why:** the user asked for weighing "like commercial kitchen scales" (photo of a stainless bench scale with a blue LCD), approved on 2026-09-19, and the earlier dial work set the pattern of no answer reveal beside the instrument.
  **How to apply:** reuse the kitchen bench-scale component for the closing waste weighing once the merging close-scene task has landed, rather than building a second scale; keep the instrument outside the live region and announce state through one status line.

- 2026-09-22 reviewer feedback (art'otel sous chef v1 docx) authorised two lexical edits to the signed `mechanic.json` beyond punctuation: the close-of-day line ends "a handover they can use to serve our guests", and the Task 3 job-card "Who you're working with" starts "Terence, who…" (no "Alongside"). Do not "restore" them. Everything else in that file changed punctuation only (Oxford commas, capital after a colon).
- Terence voice pass: rewrites must be shown to the user before being applied (asked for explicitly 2026-09-22); the recorded chill-comparison lines and the Task 4 "pending check" lines keep their wording.
