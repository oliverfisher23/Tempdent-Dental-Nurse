...
- The media component measures its slot in `useLayoutEffect` too: it remounts on every appliance change (one mount per unit across all door phases, see fridge-door-playback.md), so a passive effect flashes the default size each time.
